...
Configured covariance and forcing-error models must validate finite scales and a positive-semidefinite dependence parameterization, use disjoint reproducible random coordinates, and state the spatial and temporal support implied by numerical resampling. A user-declared correlation, error magnitude, or transition probability is a sensitivity hypothesis unless its object-, region-, and forcing-specific observational estimator and dataset provenance are archived; software support alone is not calibration evidence.
