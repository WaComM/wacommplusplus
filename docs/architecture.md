...
Projected preprocessing is an optional adapter-layer operation. PROJ transforms EPSG:4326 ocean targets into an explicitly declared source CRS, while fields remain in their normalized Earth-relative vector basis. The transformed coordinates enter the same rectilinear bilinear weights. Neither the solver nor an execution backend contains CRS logic.
