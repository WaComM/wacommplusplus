...
| Search-and-rescue uncertainty | Leeway supports ensemble properties and documented jibing behavior; plotting exposes trajectory spread. | Deterministic left/right leeway and seeded diffusion are available; backward stochastic output is explicitly an ensemble of candidate origins. Jibing and coefficient distributions are not implemented. | Parameter uncertainty and jibing are the next scientifically material SAR gaps, contingent on verified empirical distributions. |
