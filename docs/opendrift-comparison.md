...
| Products and transport | Readers include local and remote sources, including generic CF NetCDF and OPeNDAP workflows. | ROMS, NEMO, HYCOM, native WaComM, WRF, and WW3 accept local NetCDF or explicit HTTP/HTTPS/DAP4 URIs and load a bounded pair of forcing windows on demand. | Extend product coverage without weakening URI, metadata, grid, or no-fallback validation. |
| Physics catalog | Specialized models cover passive ocean drift, leeway, oil, eggs, plastics, icebergs, ships, and other applications. | One transport core covers passive particles, a broader side-specific surface-object catalog, optional explicit Stokes drift, and a machine-readable inventory of the processes already implemented. | Add new physics only with equations, units, references, restart analysis, and backend-equivalence tests. A catalog label never substitutes for an implemented and validated equation. |
...
| Visualization and interaction | Plotting, animation, GUI, notebook examples, and an extensive gallery are first-class interfaces. | NetCDF outputs and documented command-line examples are primary; a tested postprocessor produces provenance-bearing JSON summaries, accessible SVG maps, and a self-contained interactive physical-time viewer with member selection. | Extend only with explicit statistical meaning and geospatial provenance; presentation breadth is not evidence of model skill. |
...
The comparison identifies five ordered research-engineering milestones. Status refers to the repository revision containing this document:
...
3. **Implemented with opt-in regional calibration:** reproducible SAR ensembles use cataloged marginal regression residuals, a declared residual correlation, an explicit Bernoulli prior over initial side, substep-resolved wind-error perturbations, and an explicit hourly jibing probability converted through a constant exponential hazard. A bounded non-overlapping EPSG:4326 region table can replace wind-error parameters only when object-, forcing-, estimator-, period-, sample-, dataset-, checksum-, and DOI provenance is declared. Counter keys include stable identity, physical interval, substep, and process component. Numerical verification covers selection, validation, repeatability, restart, scheduling, and accelerator parity. No covariance estimate is bundled; scientific validity remains evidence supplied by the study.
4. **Implemented:** add a read-only trajectory-map and ensemble-diagnostic tool that joins stable identities across physical-time snapshots, handles the antimeridian, reports geodesic radial-spread summaries, propagates input checksums/build provenance, and emits a self-describing accessible SVG plus an optional self-contained interactive time/member view without inventing coastline data or probability contours.
5. **Implemented:** resolve local and remote environmental locations explicitly and load forcing windows on demand through one bounded current/adjacent cache. Remote and local datasets pass through the same adapters and fail-fast metadata checks; transport failure never changes the selected environmental source.
6. **Implemented:** broaden the leeway catalog across people, survival craft, small craft, vessels, and debris while retaining restart IDs 0--5 and representing distinct left/right regressions. Add a descriptive process catalog for existing operators without claiming that catalog breadth supplies oil weathering, biological behavior, iceberg dynamics, or other absent equations.
