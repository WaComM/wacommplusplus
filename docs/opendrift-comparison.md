...
| Search-and-rescue uncertainty | Leeway supports ensemble properties and documented jibing behavior; plotting exposes trajectory spread. | Deterministic left/right leeway, reproducible catalog-residual ensembles, and seeded diffusion are available; backward ensemble output is explicitly a family of candidate origins. Jibing is not implemented. | Validate ensemble calibration against observations and add jibing only with a defensible transition law. |
...
| Grid/projection mediation | Readers support multiple projections and interpolate at element locations. | All environmental adapters must already normalize onto the ocean particle grid; unequal grids fail. | Conservative regridding and projection mediation are high-value future work, but fail-fast behavior is retained until scientifically verified. |
...
2. **Partially implemented:** provide explicit rectilinear and inverse-bilinear curvilinear geographic regridding with constant preservation, analytical tests, antimeridian handling, fold detection, no extrapolation, Earth-relative vector semantics, deterministic indexed cell location, and optional declared-CRS projection through PROJ for rectilinear and curvilinear metric WRF/WW3 grids. Conservative remapping remains open and is not implied for point-sampled environmental velocity.
3. **Partially implemented:** add reproducible SAR parameter ensembles from cataloged regression residual standard deviations. Each particle receives fixed independent Gaussian downwind and crosswind residuals keyed by seed and stable identity, with forward/backward, restart, scheduling, and CPU/CUDA invariants. Empirical covariance, forcing uncertainty, randomized side, and jibing remain open; jibing requires a peer-reviewed transition model rather than an inferred implementation constant.
