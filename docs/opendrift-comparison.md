...
3. **Implemented as a calibration-ready uncertainty model:** reproducible SAR ensembles use cataloged marginal regression residuals, a declared residual correlation, an explicit Bernoulli prior over initial side, substep-resolved wind-error perturbations, and an explicit hourly jibing probability converted through a constant exponential hazard. Counter keys include stable identity, physical interval, substep, and process component. Numerical verification covers repeatability, restart, scheduling, and accelerator parity. No bundled covariance, wind-error magnitude, or transition rate is labeled observationally calibrated: scientific calibration remains dataset-, object-, and forcing-system-specific evidence that must accompany a study.
