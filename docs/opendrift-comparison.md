...
| Search-and-rescue uncertainty | Leeway supports ensemble properties and documented jibing behavior; plotting exposes trajectory spread. | Deterministic or explicitly randomized initial side, reproducible catalog-residual ensembles, and seeded diffusion are available; backward output is explicitly a family of candidate origins. Jibing is not implemented. | Validate ensemble calibration against observations and add jibing only with a defensible transition law. |
...
| Grid/projection mediation | Readers support multiple projections and interpolate at element locations. | Environmental adapters provide explicit geographic/projected interpolation; a separate rectilinear spherical operator conserves cell-average scalar integrals. | Extend conservative support only to scientifically defined curvilinear/masked quantities; retain fail-fast behavior for unsupported geometry and vector fluxes. |
...
2. **Implemented within declared scope:** provide explicit rectilinear and inverse-bilinear curvilinear geographic regridding, optional declared-CRS projection for metric WRF/WW3 grids, and separate first-order conservative spherical-overlap remapping for rectilinear geographic cell-average scalars. Point-sampled environmental velocity remains bilinear because conservative scalar remapping does not define a vector-flux operator. Curvilinear conservative polygons, masks, and second-order reconstruction are documented extensions rather than implied capabilities.
3. **Partially implemented:** add reproducible SAR parameter ensembles from cataloged regression residual standard deviations and an optional user-declared Bernoulli prior over initial crosswind side. Residuals and side use disjoint streams keyed by seed and stable identity, with forward/backward, restart, and scheduling invariants. Empirical covariance, forcing uncertainty, and jibing remain open; jibing requires a peer-reviewed transition model rather than an inferred implementation constant.
