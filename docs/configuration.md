...
With `physics.random_sources=true`, each released grid coordinate receives an independent normal offset with mean 0 and standard deviation 0.25 grid-index units. The three coordinates use disjoint stateless random components keyed by `physics.random_seed` and stable particle identity. The offsets are consequently invariant under forcing-interval partition, restart, MPI rank count, thread scheduling, and accelerator scheduling. They describe computational source-position spread in fractional grid coordinates, not a distance-isotropic or observationally calibrated source footprint.
