...
python3 tools/reproducibility_manifest.py \
  --config examples/forward-roms/forward-roms.json \
  --build-dir build \
  --source-dir . \
  --output output.nc \
  --test-log ctest.log \
