...
Select the CPU configuration \((p^*,n^*)\) with the smallest median solver time among **all** CPU points, breaking exact ties by smaller \(p\), then smaller \(n\). Report it as the overall observed CPU optimum. If that placement requires more than one 32-core GPU node, also select the fastest one-node CPU tuple \((p_g^*,n_g^*)\) as the GPU-compatible CPU reference. Execute `pg*/ng*/0`, `pg*/ng*/1`, `pg*/ng*/2`, `pg*/ng*/3`, `pg*/ng*/4` on one node; the zero-device point is its already measured CPU run. This prevents a Slurm per-node GPU request from being mislabeled as a total device count. All nonzero device counts require distinct devices and a supported binding; record rank-to-device mapping and whether devices are shared by MPI ranks. For GPU runs report incremental speedup \(T_{p_g^*,n_g^*,0}/T_{p_g^*,n_g^*,g}\). For \(g\geq1\), additionally report GPU device speedup \(G_g=T_{p_g^*,n_g^*,1}/T_{p_g^*,n_g^*,g}\) and GPU device efficiency \(G_g/g\), holding CPU resources fixed. The zero-device point has no GPU efficiency. Do **not** call this CPU parallel efficiency: a CPU thread and GPU device are not equivalent processing elements. Report GPU count, utilization and memory, PCIe/network topology, and energy if measured. A GPU point is valid only if the backend executes the intended solver kernels, rather than silently falling back to CPU.
...
5. Invoke the collector; inspect the charts, `results.json`, the overall selected \((p^*,n^*)\), and the one-node GPU reference \((p_g^*,n_g^*)\). Each selection uses median solver time within its declared candidate set.
