...
Optional switches are `USE_OMP`, `USE_MPI`, `USE_EMPI`, `USE_OPENACC`, `USE_CUDA`, and `USE_PROJ`. `USE_PROJ=ON` enables explicitly declared projected environmental grids and requires the PROJ library and coordinate database; it is not required by the portable core or geographic adapters. CUDA is unavailable on modern macOS. Configuration fails explicitly when a requested dependency is missing.
