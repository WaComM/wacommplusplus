...
For drift objects, the deterministic vector reversed by the solver is the complete `V_current + V_leeway` velocity. Leeway coefficients and wind components are not negated inside the object model. Consequently, a constant-field forward/backward round trip evaluates the same physical model in opposite temporal orientations. Any stochastic diffusion, coefficient uncertainty, unresolved forcing error, shoreline interaction, or future jibing process destroys exact pathwise reversibility and must be interpreted probabilistically.
