...
For drift objects, the deterministic vector reversed by the solver is the complete `V_current + V_leeway` velocity. Leeway coefficients and wind components are not negated inside the object model. Consequently, a constant-field forward/backward round trip evaluates the same physical model in opposite temporal orientations. A fixed coefficient-ensemble member or resolved random-side member is also reversible when seed and particle identity match, although the collection expresses conditional parameter sensitivity rather than a unique origin. Stochastic diffusion, independently resampled parameters, unresolved forcing error, shoreline interaction, or future jibing destroys exact pathwise reversibility and must be interpreted probabilistically.
