...
`examples/wacomm-sarno-lite/tools/publication_figures.py` adds read-only map panels, vertical count profiles, a final radial section, and depth summaries. The [six-hour Sarno guide](../examples/wacomm-sarno-lite/docs/README.md) gives the full command, actual results, limitations, and provenance. Its figures are [maps](../examples/wacomm-sarno-lite/docs/figures/sarno-maps.svg) and [profiles](../examples/wacomm-sarno-lite/docs/figures/sarno-profiles.svg). Install the optional pinned Python 3.11 dependencies with `pip install -r tools/requirements-figures.txt`; they are not portable-core dependencies. SVG/PDF preserve labels and contours as vectors, rasterize dense point layers, and provide white backgrounds legible in either reading theme. PNG uses 400 dpi. Root SVG titles/descriptions support accessibility.
...
## OpenMP and MPI comparison figures

`examples/wacomm-sarno-lite/tools/openmp_scaling_figures.py ROOT --mpi-root MPI_ROOT --mpi-report MPI_REPORT --output-dir OUTPUT` uses the same optional plotting environment and native-forcing/physical-interval checks as the MPI tool. It requires one process and exactly 1, 2, 4, 8, 16, and 32 OpenMP threads; it verifies allocation settings and exact particle identity/state against MPI one-process output. Inputs remain read-only, and output must be outside both simulation roots. The [example](../examples/wacomm-sarno-lite/docs/README.md#openmp-strong-scaling-and-mpi-comparison) supplies the full command, figures, and reproducibility protocol.

For dimensionless thread count $n$ and summed solver time $T_n$ in seconds, speedup is $T_1/T_n$ and efficiency is $T_1/(nT_n)$. Each backend sweep has its own one-core baseline; full-application ratios use separate application baselines. Comparisons use equal active core counts without extrapolation. The collector requires complete successful runs, positive finite intervals, identical binary/configuration/source hashes, common forcing manifests, and matching MPI log hashes. Every stored particle variable is compared with zero numeric tolerance after integer-ID ordering and physical-time matching; identical missing masks are required. Build metadata and gridded output are not part of particle equivalence. Plots show logarithmic axes, ideal reference lines, and single samples without confidence bands or inferred probabilities. Historical runs, cache/frequency variation, affinity and NUMA placement prevent causal interpretation or uncertainty estimation from these ratios alone.
