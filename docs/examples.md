...
The paired `sar-person-forward` and `sar-person-backward` examples isolate the new leeway term using uniform 10 m wind. They are verification scenarios, not operational SAR forecasts. The historical files whose names contain `sar` retain their original passive source/restart role unless their JSON explicitly selects `drift.model=leeway`.
