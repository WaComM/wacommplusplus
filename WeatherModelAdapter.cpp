...
void WeatherModelAdapter::regridBilinearCurvilinearGeographic(const Array2<double>& targetLon,const Array2<double>& targetLat) {
    Array3<float> u=EnvironmentalRegridder::bilinearCurvilinearGeographic(Lon(),Lat(),WindU10(),targetLon,targetLat);
    Array3<float> v=EnvironmentalRegridder::bilinearCurvilinearGeographic(Lon(),Lat(),WindV10(),targetLon,targetLat);
    Lon().Deallocate(); Lon().Allocate(targetLon.Nx(),targetLon.Ny()); Lon().Load(targetLon());
    Lat().Deallocate(); Lat().Allocate(targetLat.Nx(),targetLat.Ny()); Lat().Load(targetLat());
    WindU10().Deallocate(); WindU10().Allocate(u.Nx(),u.Ny(),u.Nz()); WindU10().Load(u());
    WindV10().Deallocate(); WindV10().Allocate(v.Nx(),v.Ny(),v.Nz()); WindV10().Load(v());
