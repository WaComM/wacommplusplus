...
Build with `cmake -S . -B build && cmake --build build`, then run `./build/wacommplusplus examples/restart-backward/restart-backward.json`. The solver traverses files and records from newer to older, skipping intervals newer than the checkpoint and clipping the containing interval.
