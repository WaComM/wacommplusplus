...
The JSON selects `NEMO`, enables sources, disables stochastic motion, records seed `5489`, limits particle steps to 30 s, and traverses forcing forward. Replace paths, then run `cmake -S . -B build && cmake --build build` and `./build/wacommplusplus examples/forward-nemo/forward-nemo.json`.
