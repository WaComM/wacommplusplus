...
```bash
ln -sf ../examples/sources-webinar/sources-webinar.json sources.json
```
...
```bash
ln -sf ../examples/webinar-roms-usecase-download/webinar-roms-usecase-download.json wacomm.json
```
```bash
./wacommplusplus
```
...
```bash
ln -sf ../examples/webinar-native-usecase/webinar-native-usecase.json wacomm.json
```
...
```bash
sbatch --gres=gpu:tesla:1 --job-name=wpp1G --output=results/wppDemo.out --error=results/wppDemo.err -c 1 -n 1 ../examples/ADMIRE-WP7-webinar-demo/slurm_webinarGPU.sh
```
...
...
```
![Output example of first hour of simulation (09:00 UTC) plotted using GrADS](docs/figures/ist09.png)
Output example of first hour of simulation (09:00 UTC) plotted using GrADS
![Another exaple of drifter simulation (12:00 UTC) plotted using GrADS](docs/figures/ist12.png)
