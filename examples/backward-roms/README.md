...
`backward-roms.json` selects the ROMS adapter, deterministic motion, `direction:backward`, `backward_diffusion:none`, and endpoint restart input. Replace paths, build with `cmake -S . -B build && cmake --build build`, then run `./build/wacommplusplus examples/backward-roms/backward-roms.json`.
