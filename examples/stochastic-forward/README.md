...
Configure and run with `cmake -S . -B build && cmake --build build` followed by `./build/wacommplusplus examples/stochastic-forward/stochastic-forward.json`. Repeating the run with the same inputs, executable, and seed must reproduce particle state; changing the seed should change stochastic displacement without changing emissions.
