...
Configure with `cmake -S . -B build-mpi -DUSE_MPI=ON`, build with `cmake --build build-mpi`, then run `mpiexec -n 4 ./build-mpi/wacommplusplus examples/parallel-mpi/parallel-mpi.json`. MPI partitions particles while the adapter and numerical equations remain unchanged.
