...
The leeway-enabled reference set includes catalog-mean, coefficient-ensemble, crosswind-side ensemble, coupled WRF/WW3, and projected-wave forward/backward pairs. The side examples declare a Bernoulli prior rather than an inferred empirical frequency. `sar-person-projected-wave-forward` and `sar-person-projected-wave-backward` require `USE_PROJ=ON` and demonstrate an explicit EPSG:3857 WW3 grid without implying that Web Mercator is scientifically preferred. Other historical files containing `sar` remain passive unless their JSON explicitly selects `drift.model=leeway`.
