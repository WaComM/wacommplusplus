...
The leeway-enabled reference set is `sar-person-forward`, `sar-person-backward`, `sar-person-ensemble-forward`, `sar-person-ensemble-backward`, `sar-person-wrf-ww3-forward`, and `sar-person-wrf-ww3-backward`. The first pair combines ROMS current with uniform 10 m wind, the ensemble pair samples fixed empirical regression residuals, and the coupled pair uses WRF wind and WW3 surface Stokes drift through model-independent environmental interfaces. Other historical files containing `sar` in their name remain passive unless their JSON explicitly contains `"drift":{"model":"leeway",...}`. This distinction prevents a scenario label from silently changing its governing equations.
