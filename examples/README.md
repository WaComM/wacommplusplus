...
The leeway-enabled reference set is `sar-person-forward` and `sar-person-backward`. These examples combine ROMS current with a uniform 10 m wind through the direction-neutral surface-drift model. Other historical files containing `sar` in their name remain passive unless their JSON explicitly contains `"drift":{"model":"leeway",...}`. This distinction prevents a scenario label from silently changing its governing equations.
