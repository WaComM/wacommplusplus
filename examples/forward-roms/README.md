...
`forward-roms.json` selects the ROMS adapter, enables ordinary sources, disables stochastic displacement, records seed `5489`, uses `dti=30 s` as a maximum step, and selects forward tracking. Replace paths, then run `cmake -S . -B build && cmake --build build` and `./build/wacommplusplus examples/forward-roms/forward-roms.json`.
