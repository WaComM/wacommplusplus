...
The JSON selects the `WACOMM` adapter, deterministic forward tracking, source emission, and a maximum 30 s step. Replace paths, run `cmake -S . -B build && cmake --build build`, then `./build/wacommplusplus examples/forward-wacomm/forward-wacomm.json`.
