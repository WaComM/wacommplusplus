...
if [[ ! "$suite_name" =~ ^[a-zA-Z0-9-]+$ ]] || [ "$#" -gt 2 ]; then
    echo 'Usage: bash examples/wacomm-sarno-lite/tools/resume_sarno_protocol_cpu.sh suite-name [partition]' >&2
    exit 1
fi
suite="$root/$suite_name"
test -f "$suite/provenance/config.json"
cp "$example/tools/sarno_protocol_job.sh" "$suite/provenance/sarno_protocol_job-resume.sh"
cp "$example/tools/resume_sarno_protocol_cpu.sh" "$suite/provenance/"
...
for repetition in 1 2 3; do
    ...
    for ((index=0; index<count; index++)); do
        ...
        job=$(sbatch --parsable --partition="$partition" --nodes="$nodes" --ntasks="$processes" \
            ...
            --export="ALL,SARNO_PROCESSES=$processes,SARNO_THREADS=$threads,SARNO_GPUS=0,SARNO_REPETITION=$repetition" \
