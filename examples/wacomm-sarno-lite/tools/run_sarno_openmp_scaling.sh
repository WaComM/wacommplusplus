...
partition="${1:-high-wn}"
if [ "$#" -gt 1 ]; then echo "Usage: bash examples/wacomm-sarno-lite/tools/run_sarno_openmp_scaling.sh [partition]" >&2; exit 1; fi
...
cp "$repository/examples/wacomm-sarno-lite/tools/sarno_openmp_scaling_job.sh" "$suite/provenance/"
cp "$repository/examples/wacomm-sarno-lite/tools/run_sarno_openmp_scaling.sh" "$suite/provenance/"
...
for threads in 1 2 4 8 16 32; do
    ...
    cp "$repository/examples/wacomm-sarno-lite/tools/sarno_openmp_scaling_job.sh" "$run/submit.sh"
