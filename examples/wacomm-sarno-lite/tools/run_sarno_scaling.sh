...
partition="${2:-high-wn}"
if [ "$#" -gt 2 ] || [[ ! "$suite_name" =~ ^[a-zA-Z0-9-]+$ ]]; then echo "Usage: bash examples/wacomm-sarno-lite/tools/run_sarno_scaling.sh [suite-name [partition]]" >&2; exit 1; fi
...
cp "$repository/examples/wacomm-sarno-lite/tools/sarno_scaling_job.sh" "$suite/provenance/"
cp "$repository/examples/wacomm-sarno-lite/tools/run_sarno_scaling.sh" "$suite/provenance/"
