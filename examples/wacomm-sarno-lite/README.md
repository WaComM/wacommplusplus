...
These observations describe the archived pre-fix run. Subsequent metric and interpolation corrections passed the synthetic CPU/CUDA particle parity test on `gn03` in Slurm job 6358. The complete Sarno CPU/CUDA application comparison has not been rerun with those corrections, so the archived figures remain computational diagnostics.
